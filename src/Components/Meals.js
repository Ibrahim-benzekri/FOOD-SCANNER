...
const Meals = () => {
  ...
  const { ingredient } = useParams(); // Access the URL parameter
  const { group } = useParams(); 
const navigate = useNavigate();
  useEffect(() => {
    if (ingredient) {
        fetchMeals(ingredient); // Trigger a search when the ingredient changes
    }
  }, [ingredient]);
  useEffect(() => {
    if (group) {
      fetchMeals(group); // Trigger a search when the group changes
    }
  }, [group]);

  const fetchMeals = (group) => {
    fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?i=${group}`)
      .then((res) => res.json())
      .then((data) => {
        setMeals(data.meals || []);
      });
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-gray-50">
      <div className="max-w-screen-xl mx-auto p-8 bg-white text-center shadow-md">
        ...
        <div className="mt-16">
          <h2 className="text-2xl font-bold mb-4">Your Search Results For {ingredient}:</h2>
          <div
            id="meal"
            className={`grid gap-6 ${
              meals.length === 0 ? 'text-orange-600 font-semibold' : 'sm:grid-cols-2 lg:grid-cols-3'
            }`}
          >
            {meals.length > 0 ? (
              meals.map((meal) => (
  ...
  <div className="fixed inset-0 bg-orange-600 bg-opacity-90 flex items-center justify-center">
    <div className="relative w-11/12 max-w-2xl h-auto max-h-screen overflow-y-auto rounded-lg bg-white p-8 shadow-lg">
      ...
      >
        <i class="font-normal text-2xl text-black">X</i>
      </button>
